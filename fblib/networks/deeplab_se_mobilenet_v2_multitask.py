...
import fblib.networks.se_mobilenet_v2 as se_mobilenet_v2_imagenet
...
class SEMobileNetV2(nn.Module):
    def __init__(self, n_classes, width_mult=1., output_stride=16,
                 ...
                 dscr_type=None, dscr_k=1, dscr_d=2):
        ...
        if self.use_dscr:
            self.dscr_k = dscr_k
            self.dscr_d = dscr_d
            print('Using Discriminator type: {} '
                  'with kernel size: {} and depth: {}'.format(dscr_type, self.dscr_k, self.dscr_d))
            self.discriminator = self._get_discriminator(dscr_type, width_decoder=64)
...
def test_vis_net(net, elems):
    ...
    dataset = PASCALContext(split=['train'], transform=transform,
                            retname=True,
                            do_edge=True,
                            do_human_parts=True,
                            do_semseg=True)
    ...
    img = img.cuda()
    y, _ = net.forward(img, task=elems[-1])
