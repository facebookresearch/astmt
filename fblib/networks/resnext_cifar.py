...
class CifarResNeXt(nn.Module):
  ...
  """
    ...
    def __init__(self, block, depth, cardinality, base_width, num_classes):
        ...
        self.inplanes = 64
        self.stage_1 = self._make_layer(block, 64, layer_blocks, 2)
        self.stage_2 = self._make_layer(block, 128, layer_blocks, 2)
        self.stage_3 = self._make_layer(block, 256, layer_blocks, 1)
...
def resnext20_16_64(num_classes=10):
    """Constructs a ResNeXt-18, 16*64d model for CIFAR-10 (by default)

    Args:
      num_classes (uint): number of classes
    """
    model = CifarResNeXt(ResNeXtBottleneck, 20, 16, 64, num_classes)
    return model


def resnext20_8_64(num_classes=10):
    """Constructs a ResNeXt-18, 16*64d model for CIFAR-10 (by default)

    Args:
      num_classes (uint): number of classes
    """
    model = CifarResNeXt(ResNeXtBottleneck, 20, 8, 64, num_classes)
    return model
