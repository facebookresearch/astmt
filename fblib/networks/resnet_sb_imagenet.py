...
def center_crop(x, height, width):
    ...
    return torch.nn.functional.pad(x, [int(crop_w.ceil()[0]), int(crop_w.floor()[0]),
                              int(crop_h.ceil()[0]), int(crop_h.floor()[0])])
...
class SBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, group_norm=False):
        ...
        self.bn1 = norm2d(planes, group_norm)
        self.conv2 = nn.Conv2d(planes, planes // 2, kernel_size=3, padding=1, stride=2, bias=False)
        self.deconv2 = nn.ConvTranspose2d(planes // 2, planes, kernel_size=4, padding=1, stride=2)
        self.bn2 = norm2d(planes, group_norm)
    ...
    def forward(self, x):
        ...
        if out.shape[2] != w or out.shape[3] != h:
            out = center_crop(out, w, h)
        out = self.bn2(out)
        out = self.relu(out)
...
def resnet18(pretrained=False, features=False, **kwargs):
    ...
    """
    if not features:
        model = ResNet(SBasicBlock, [2, 2, 2, 2], **kwargs)
    else:
        model = ResNetFeatures(SBasicBlock, [2, 2, 2, 2], **kwargs)
...
def resnet34(pretrained=False, features=False, **kwargs):
    ...
    """
    if not features:
        model = ResNet(SBasicBlock, [3, 4, 6, 3], **kwargs)
    else:
        model = ResNetFeatures(SBasicBlock, [3, 4, 6, 3], **kwargs)
...
def resnet26(pretrained=False, features=False, **kwargs):
    ...
    """
    if not features:
        model = ResNet(SBottleneck, [2, 2, 2, 2], **kwargs)
    else:
        model = ResNetFeatures(SBottleneck, [2, 2, 2, 2], **kwargs)
...
def resnet50(pretrained=False, features=False, **kwargs):
    ...
    """
    if not features:
        model = ResNet(SBottleneck, [3, 4, 6, 3], **kwargs)
    else:
        model = ResNetFeatures(SBottleneck, [3, 4, 6, 3], **kwargs)
...
def resnet101(pretrained=False, features=False, **kwargs):
    ...
    """
    if not features:
        model = ResNet(SBottleneck, [3, 4, 23, 3], **kwargs)
    else:
        model = ResNetFeatures(SBottleneck, [3, 4, 23, 3], **kwargs)
...
def resnet152(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(SBottleneck, [3, 8, 36, 3], **kwargs)
    if pretrained:
...
def test_reproduce():
    ...
    model = resnet50(pretrained=False, features=False)
...
if __name__ == '__main__':
    print('Hi Ilija, please train us')
    net = resnet26(pretrained=False)
