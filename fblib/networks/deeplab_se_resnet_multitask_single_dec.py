...
class ResNet(nn.Module):
    def __init__(self, block, layers, n_classes, nInputChannels=3, classifier='atrous-v3',
        ...
        if self.casc_type == 'early' or self.casc_type == 'mix':
            print('Initializing {} cascading residual layers on Encoder'.format(self.casc_type))
            n_latent = 512
            n_in = 2048 if self.casc_type == 'early' else width_decoder
            stride_casc = output_stride // 4 if self.casc_type == 'mix' else 1
            self.residual_cascade = nn.Sequential(nn.Conv2d(n_in, n_latent, kernel_size=1, stride=stride_casc),
                                                  nn.ReLU(inplace=True),
                                                  nn.Conv2d(n_latent, n_latent, kernel_size=3, padding=1),
                                                  nn.ReLU(inplace=True),
                                                  nn.Conv2d(n_latent, 2048, kernel_size=1),
                                                  )
        elif self.casc_type == 'mixcon' or self.casc_type == 'mixprev':
            print('Initializing {} cascading residual layers on Encoder'.format(self.casc_type))
            n_in = width_decoder
            n_latent = 512  # magic number
            stride_casc = output_stride // 4
            self.residual_cascade = nn.ModuleDict()
            for i_task, task in enumerate(self.tasks[1:]):
                # Skip the first task that has no cascades
                self.residual_cascade[task] = nn.Sequential(nn.Conv2d(n_in * len(casc_tasks[task]), n_latent,
