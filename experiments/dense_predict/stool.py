...
from experiments.dense_predict import MAX_N_IMAGES_PER_GPU


def parse_json(grid, n_gpu_param=None):
    ...
    for p in perms:
        ...
        for i, k in enumerate(grid.keys()):
            if type(p[i]) is int or type(p[i]) is float:
                ...
                v = '"%s"' % p[i]
            key = str(k)
            if n_gpu_param is not None and key == n_gpu_param:
                s_name = p[list(grid.keys()).index('arch')] + '-' + str(p[list(grid.keys()).index('stride')])
                n_gpus.append((int(p[i]) - 1) // MAX_N_IMAGES_PER_GPU[s_name] + 1)
            if len(key) > 2:
                argstr += " --%s %s" % (str(k), v)
            else:
                argstr += " -%s %s" % (str(k), v)
...
parser.add_argument('-C', type=str, default='volta')
...
if sys.argv[-1][-3:] == '.sh':
    ...
    module = importlib.import_module('experiments.dense_predict.' + EXPERIMENT)
    global MAX_N_IMAGES_PER_GPU
    MAX_N_IMAGES_PER_GPU = module.MAX_N_IMAGES_PER_GPU
...
if COMMAND == 'run':
    ...
    with open(args.sweep, 'r') as f:
        [sweep_commands, n_gpus] = parse_json(json.loads(f.read()), 'trBatch')
