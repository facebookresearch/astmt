...
def parse_args():
    ...
    parser.add_argument('--taskstg', type=int, default=1,
                        help='How many task specific stages? Default 1, maximum 3')
    ...
    parser.add_argument('--adj_w', type=str2bool, default=False,
                        help='adjust weights of tasks according to the gradient norm')
...
def create_config():
    ...
    args.save_path = os.path.join(Path.exp_dir(), 'cifar_stg', p.exp_names)
...
def main():
    ...
    if args.arch == 'x20':
        net = resnext_multitask.resnext20(base_width=args.width, num_classes=args.n_outputs, n_tasks=n_aux_tasks,
                                          ...
                                          xpath=args.xpath, n_gpu=args.n_gpu, task_stages=args.taskstg,
                                          ret_features=True)
    elif args.arch == 'x29':
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for task in range(args.n_tasks):
            if task in args.active_tasks:
                is_best[task] = recorder.update(epoch, train_los[task],
                                                train_acc[task], val_los[task], val_acc[task], task)
...
def train(train_loader, model, criterion, optimizer, epoch, log, args, bin_wrapper):
    ...
    for i, sample in enumerate(train_loader):
        ...
        # compute output
        output, features = model(input_var, args.active_tasks)
        losses_tasks = model.compute_losses(output, features, criterion, target_var, task_labels, args)
        loss = 0
        for task in range(args.n_tasks):
            if task in args.active_tasks:
                # print(task)
                loss += args.main_w[task] * losses_tasks[task]
                loss_task[task] = losses_tasks[task].item()

                # measure accuracy and record loss
                prec1, prec5 = accuracy(output[task].data, target_var[task], topk=(1, 5))
                losses[task].update(loss_task[task], input_var[task].size(0))
        ...
        if task in args.active_tasks:
            print_log(
                '  **Train** Task: {} Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f} Error@1 {error1:.3f}'.format(
                    task, top1=top1[task], top5=top5[task], error1=100 - top1[task].avg),
                log)
...
def validate(val_loader, model, criterion, log, args, bin_wrapper=[]):
    ...
    with torch.no_grad():
        for i, sample in enumerate(val_loader):
            ...
            for task in range(args.n_tasks):
                if task in args.active_tasks:
                    curr_loss = criterion(output[task], target_var[task])
                    loss_task[task] = curr_loss.item()
