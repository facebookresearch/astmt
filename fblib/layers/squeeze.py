...
class SELayerMultiTask(nn.Module):
    def __init__(self, channel, reduction=16, n_tasks=None):
        super(SELayerMultiTask, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        ...
        if n_tasks is None:
            ...
            print('Initializing {} squeeze and excitation modules'.format(n_tasks))
            self.fc = nn.ModuleList([SequentialMultiTask(nn.Linear(channel, channel // reduction),
                                                         nn.ReLU(inplace=True),
                                                         nn.Linear(channel // reduction, channel),
                                                         nn.Sigmoid())
...
class ConvSELayerMultiTaskDict(nn.Module):
    def __init__(self, channel, reduction=16, tasks=None):
        super(ConvSELayerMultiTaskDict, self).__init__()

        if tasks is None:
            self.fc = nn.Sequential(
                nn.Conv2d(channel, channel // reduction, kernel_size=1),  # Reduce
                nn.BatchNorm2d(channel // reduction),
                nn.ReLU(inplace=True),

                nn.Conv2d(channel // reduction, channel // reduction, kernel_size=3, padding=1),  # Process
                nn.BatchNorm2d(channel // reduction),
                nn.ReLU(inplace=True),

                nn.Conv2d(channel // reduction, channel, kernel_size=1),  # Expand
                nn.Sigmoid())
        else:
            print('Initializing squeeze and excitation modules:')
            self.fc = nn.ModuleDict()
            for task in tasks:
                print('Fully Convolutional SE for task: {}'.format(task))

                # Attention! Don't mind code repetition, don't init the layers before sequential!
                self.fc[task] = SequentialMultiTask(
                    nn.Conv2d(channel, channel // reduction, kernel_size=1),  # Reduce
                    nn.BatchNorm2d(channel // reduction),
                    nn.ReLU(inplace=True),
...
class ConvCoupledSE(nn.Module):
    ...
    Possibility to also use fully-convolutional SE modules
    """
