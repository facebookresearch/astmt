...
class HED(nn.Module):
    ...
    """
    def __init__(self, pretrained='imagenet-caffe', n_classes=1):
        ...
        print("Constructing HED architecture..")
        ...
        for i in range(0, len(lay_list)):
            ...
            score_dsn.append(nn.Conv2d(16, n_classes, kernel_size=1, padding=0))
            upscale_.append(nn.ConvTranspose2d(n_classes, n_classes, kernel_size=2 ** (1 + i),
        ...
        self.fuse = nn.Conv2d(16 * len(score_dsn), n_classes, kernel_size=1, padding=0)
    ...
    def forward(self, x):
        ...
        for i in range(0, len(self.stages)):
            x = self.stages[i](x)
            side_temp = self.side_prep[i](x)
            side.append(center_crop(self.upscale[i](side_temp), crop_h, crop_w))
            side_out.append(center_crop(self.upscale_[i](self.score_dsn[i](side_temp)), crop_h, crop_w))
    ...
    def _initialize_weights(self, pretrained):
        ...
        if pretrained == 'imagenet-pytorch':
            ...
            for i in range(len(self.stages)):
                for j in range(len(self.stages[i])):
                    if isinstance(self.stages[i][j], nn.Conv2d):
                        ...
                        k += 1
        elif pretrained == 'imagenet-caffe':
...
if __name__ == "__main__":
    ...
    net = HED(pretrained='imagenet-caffe')
    net.to(device)
    net.eval()
