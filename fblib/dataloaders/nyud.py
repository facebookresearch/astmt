...
def test_mt():
    transform = transforms.Compose([tr.RandomHorizontalFlip(),
                                    ...
                                    tr.AddIgnoreRegions(),
                                    tr.ToTensor()])
    dataset = NYUD_MT(split=['train', 'val'], transform=transform, retname=True,
                      do_edge=True,
                      do_semseg=True,
                      do_normals=True,
                      do_depth=True)
...
def test_nrm():
    transform = transforms.Compose([tr.RandomHorizontalFlip(),
                                    ...
                                    tr.AddIgnoreRegions(),
                                    tr.ToTensor()])
    dataset = NYUDRaw(split='train', transform=transform, retname=True)

    dataloader = torch.utils.data.DataLoader(dataset, batch_size=8, shuffle=False, num_workers=16)

    for i, sample in enumerate(dataloader):
        print(i)
        image = np.transpose(sample['image'].numpy(), [2, 3, 1, 0])
        normals = np.transpose(sample['normals'].numpy(), [2, 3, 1, 0])

        for j in range(image.shape[-1]):
            figure()
            imshow(image[:, :, :, j] / 255)
            figure()
            imshow((1 + normals[:, :, :, j]) / 2.)
        show()
